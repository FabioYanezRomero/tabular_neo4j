...
LLM_API_KEY = os.environ.get(
    "LLM_API_KEY", ""
)  # Not used with LM Studio but kept for compatibility
TARGET_HEADER_LANGUAGE = os.environ.get("TARGET_HEADER_LANGUAGE", "English")
DEFAULT_LLM_PROVIDER = os.environ.get("LLM_PROVIDER", "lmstudio")
DEFAULT_SEED = int(os.environ.get("LLM_DEFAULT_SEED", 42))
DEFAULT_TEMPERATURE = float(os.environ.get("LLM_DEFAULT_TEMPERATURE", 0.0))
...
LMSTUDIO_BASE_URL = os.environ.get(
    "LMSTUDIO_BASE_URL",
    f"http://{LMSTUDIO_HOST}:{LMSTUDIO_PORT}/{LMSTUDIO_API_VERSION}",
)


def get_model_from_env(state_name: str, default: str) -> str:
    """Return model name overridden via environment variable."""
    env_var = f"LMSTUDIO_MODEL_{state_name.upper()}"
...
LLM_CONFIGS = {
    ...
    "infer_header": {
        ...
        "model_name": get_model_from_env(
            "infer_header", "Mistral-7B-Instruct-v0.2-GGUF"
        ),
        "temperature": 0.0,
        "seed": 42,
    ...
    "validate_header": {
        "provider": "lmstudio",
        "model_name": get_model_from_env("validate_header", "Llama-2-7B-Chat-GGUF"),
        "temperature": 0.0,
        "seed": 42,
    ...
    "translate_header": {
        "provider": "lmstudio",
        "model_name": get_model_from_env(
            "translate_header", "Mistral-7B-Instruct-v0.2-GGUF"
        ),
        "temperature": 0.0,
        "seed": 42,
    ...
    "analyze_column_semantics": {
        "provider": "lmstudio",
        "model_name": get_model_from_env(
            "analyze_column_semantics", "Llama-2-13B-Chat-GGUF"
        ),
        "temperature": 0.0,
        "seed": 42,
    ...
    "classify_entities_properties": {
        "provider": "lmstudio",
        "model_name": get_model_from_env(
            "classify_entities_properties", "Llama-2-13B-Chat-GGUF"
        ),
        "temperature": 0.0,
        "seed": 42,
    ...
    "reconcile_entity_property": {
        "provider": "lmstudio",
        "model_name": get_model_from_env(
            "reconcile_entity_property", "Llama-2-13B-Chat-GGUF"
        ),
        "temperature": 0.0,
        "seed": 42,
    ...
    "map_properties_to_entity": {
        "provider": "lmstudio",
        "model_name": get_model_from_env(
            "map_properties_to_entity", "Llama-2-13B-Chat-GGUF"
        ),
        "temperature": 0.0,
        "seed": 42,
    ...
    "infer_entity_relationships": {
        "provider": "lmstudio",
        "model_name": get_model_from_env(
            "infer_entity_relationships", "Llama-2-13B-Chat-GGUF"
        ),
        "temperature": 0.0,
        "seed": 42,
    ...
    "generate_cypher_templates": {
        "provider": "lmstudio",
        "model_name": get_model_from_env(
            "generate_cypher_templates", "Llama-2-13B-Chat-GGUF"
        ),
        "temperature": 0.0,
        "seed": 42,
    ...
    "synthesize_final_schema": {
        "provider": "lmstudio",
        "model_name": get_model_from_env(
            "synthesize_final_schema", "Llama-2-13B-Chat-GGUF"
        ),
        "temperature": 0.0,
        "seed": 42,
...
UNIQUENESS_THRESHOLD = (
    0.9  # Threshold for considering a column as potentially unique identifier
)
LOW_CARDINALITY_THRESHOLD = (
    0.1  # Threshold for considering a column as low cardinality (categorical)
)
