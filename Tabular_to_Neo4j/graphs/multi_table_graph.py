...
def run_multi_table_pipeline(state: MultiTableGraphState, config: Optional[Dict[str, Any]] = None) -> MultiTableGraphState:
    ...
    for table_name, table_state in state.items():
        ...
        try:
            ...
            table_state = compiled_graph.invoke(table_state, config)
            # Accept AddableValuesDict, GraphState, or any MutableMapping as valid state types
            state[table_name] = table_state
            # Log type and truncated value after node execution
            short_val = str(table_state)
            if len(short_val) > 300:
                short_val = short_val[:300] + '...'
            logger.info(f'[PER_TABLE][{table_name}][NODE_EXEC][AFTER] Type: {type(table_state).__name__}, Value: {short_val}')
            logger.info(f'[PER_TABLE][{table_name}][AFTER_PIPELINE] State type: {type(table_state).__name__}')
        except Exception as e:
            logger.error(f'[PER_TABLE][{table_name}][PIPELINE_EXCEPTION] Exception: {e}, State type: {type(table_state).__name__}, State value: {table_state}')
            raise
